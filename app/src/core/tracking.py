...
class Tracking:
    ...
    def _track_color(self, track_id):
        ...
        frames.  Avoids hues near pure green (OpenCV H 35–85, ≈ 70°–170°
        standard) to ensure contrast on grass backgrounds.
        ...
        # Deterministic hue in OpenCV's 0-179 range. Prime multiplier (47)
        # spreads hues across small sequential track IDs (1, 2, 3, …).
        hue = (int(track_id) * 47) % 180
        # Shift the green band (35-85 ≈ 70°-170°) away from grass.
        if 35 <= hue <= 85:
            hue = (hue + 100) % 180
        saturation = 220
        value = 200
    ...
    def plot_one_box(self, x, img, color=None, label_id=None, label_score=None, line_thickness=None):
        ...
        Draws:
        (a) A 1px dark outline rectangle behind the colored stroke for contrast
            on any background.
        (b) A colored anti-aliased rectangle at ``line_thickness``.
        (c) A dark semi-transparent rounded badge above the box (flipped below
            if near the frame top) sized to fit the label text.
        (d) ``label_id`` text (e.g. "#12") in a larger fontScale and thicker
            stroke (simulated bold).
        (e) ``label_score`` text (e.g. "0.87") smaller and thinner to the right
            of the ID.
        ...
        if label_id or label_score:
            ...
            # Badge above the box by default; flip below if it would overflow the top.
            badge_x1 = c1[0]
            ...
            # (c) Dark semi-transparent badge via addWeighted on the sub-ROI
            ...
            # (d) #ID text — larger, thicker (simulated bold) + (f) shadow
            if id_text:
            ...
            # (e) Score text — smaller, thinner, to the right of ID + (f) shadow
            if score_text:
