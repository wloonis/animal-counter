...
- [ ] **Task 1: ADD** `app/src/main.py` — Add `_finalize_recording(self)` method to the `DisplayThread` class. The method must:
...
- [ ] **Task 4: REMOVE** `app/src/main.py` (~495-496) — Remove the 'q' key handler in CAMERA mode. The current code block is:
  ```python
...
- [ ] **Task 5: MODIFY** `app/src/main.py` (~58-79) — Modify `stop()` to call `_finalize_recording()` on the display thread instead of the raw release+null. The current code is:
  ```python
  def stop():
